...
class KANet5ConvConv(nn.Module):
    def __init__(self, in_channels=1, out_channels=6, kernel_size=3, second_out = 16, order=(5, 4)):
        ...
        self.conv1 = KAConv(in_channels, out_channels // 2, kernel_size, mode=['identity'], order=order)
        self.bn1 = nn.BatchNorm2d(out_channels // 2)

        self.conv2 = KAConv(out_channels //2, out_channels, kernel_size, "identity", order=order)
        self.bn2 = nn.BatchNorm2d(out_channels)

        self.conv3 = KAConv(out_channels, second_out // 2, kernel_size, mode=['identity'], order=order)
        self.bn3 = nn.BatchNorm2d(second_out // 2)

        self.conv4 = KAConv(second_out // 2, second_out, kernel_size, "identity", order=order)
